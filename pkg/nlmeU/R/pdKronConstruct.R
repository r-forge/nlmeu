   ...
   form = NULL, nam = NULL, data = sys.frame(sys.parent())) 
{
   ...
   .traceR(1, , funNm, "pdKronecker STARTS here")
    
    object <- numeric(0)
    class(object) <- c("pdKronecker", "pdMat")
    .traceR(1, , funNm, "pdKronecker ENDS here")
    ...
    pdClass = sapply(object,FUN=function(el) class(el)[1]) , ...) 
{
   ...
   .traceR <-   if (is.null(.traceRinit))
    ...
    if (inherits(value, "pdMat")) {
        ...
        if (isInitialized(value)) {
            .traceR(9001, , funNm, "pdConstruct.pdKronecker EXIT1")
            return(pdConstruct(object, as.matrix(value), form, 
                nam, data, pdClass))
        }
        else {
            .traceR(9001, , funNm, "pdConstruct.pdKronecker EXIT2")
            return(pdConstruct(object, form = form, nam = nam, 
                data = data, pdClass = pdClass))
        }
    }
    .traceR(9002, , funNm, "AFTER if value inherits pdMat")
    ...
    nB <- max(c(nF, nN, nP))
    .traceR(9002, , funNm, "oVal created")
   ...
   .traceR(9002, , funNm, "object initiated")
    ...
    .traceR(9002, , funNm, "End of nB loop")
...
### Inserted Ends
   .traceR(101, , funNm, "Before namesList")
    namesList <- lapply(object, Names)
    .traceR(101, , funNm, "After namesList")  
    ...
    if ((sum(namInterc) == length(lNam)) || !any(lNam[!namInterc])) {
        ...
        names(object) <- nmsObject  # Added Nov.20, 2008
        .traceR(9001, , funNm, "pdConstruct.pdKronecker EXIT4")
        return(object)
    }
}
