...
CholD <- chol(Dmtx)             # Cholesky factor U of D
vd <- svd(CholD, nu=0)          # SVD of U
vd$v %*% (log(vd$d) * t(vd$v))  # (13.41)
...
coef(pdLCh)                     # Unconstrained theta_D  
LChD  <- CholD                  # U
...
### code chunk: R14.8
###################################################
reSt  <- reStruct(list(g1 = pdSm, g2 = pdCSf))         # reStruct
corSt <- corExp(c(0.3, 0.1), form=~tx, nugget = TRUE)  # corStruct 
vF    <- varExp(0.8, form = ~agex)                     # varFunc
(lmeSt<- lmeStruct(reStruct = reSt, corStruct=corSt,   # lmeStruct
