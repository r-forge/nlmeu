...
### code chunk: R12.3b
###################################################
anova(fm9.2, fm12.1)                   # M9.3 nested in M12.1
...
myPanel <- function(x, y, subscripts, ...) {
           ...
           y1 <- y[gins]
           subs1 <- subscripts[gins]
           # tt <- cbind(grps[subs1], subs1, x1, y1)
...
panel.bwxplot <- function(x,y, subscripts, ...){
 ...
 bwplot(                                             # 12.7
    ...
    panel = panel.bwxplot,                           # User defined panel (not shown)
...
### code chunk: R12.12
###################################################
fm12.5vcov <- getVarCov(fm12.5,          # \zi{\widehat{\calR}}{i}
     individual="2")
dimnames(fm12.5vcov) <- dnms             # Dimnames assigned
fm12.5vcov 
fm12.5cor <- cov2cor(fm12.5vcov)         # \zi{\widehat{\bm{C}}}{i}:(\ref{armd:eqn:corSymm})
